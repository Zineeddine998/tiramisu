...
namespace tiramisu::auto_scheduler
{
    ...
    std::vector<syntax_tree *> ml_model_schedules_generator::generate_schedules(syntax_tree &ast)
    {
        ...
        optimization_type current_optimization = ast.get_current_optimization_type();
        // Generate the specified optimization
        switch (current_optimization)
        {
        ...
        case optimization_type::FUSION:
            ...
            /* iteration of the ast in done preorder  */
            {
                ...
                if ((previous_node != current_node) && (previous_node_adjusted == previous_node) && (previous_node != nullptr) && (current_node != nullptr))
                {
                    ...
                    { // similar itr domains
                        ...
                        if (shifting_res.size() > 0)
                        {
                            ...
                            for (auto &shifting : shifting_res)
                            {
                                ...
                                {
                                    int depth = new_node->computations[node_computation.second].comp_ptr->get_loop_level_number_from_dimension_name(std::get<0>(shifting).get_name());
                                    optimization_info optim_info;
        ...
        if (optimization_duration_map.find(current_optimization) != optimization_duration_map.end())
        {
            ...
            optimization_duration_map.at(current_optimization) = prev_duration + duration;
        }
        else
        {
        ...
        while (i < matrices.size() && matrices.at(i).size() != 0)
        {
            ...
            for (int s = 0; s < depth; s++)
            {
                for (int d = 0; d < depth; d++)
                {
                    ...
                    if (matrix.at(s).at(d) != matrices.at(i).at(s).at(d))
                        diffrent = true;
                }
            }
