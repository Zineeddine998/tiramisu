...
namespace tiramisu
{
    ...
    void tiramisu::function::reset_computations()
    {
        ...
        for (computation *comp : get_computations())
        {
            ...
            if (comp->name.substr(0, 1) == "_" && pos != std::string::npos)
            {
...
#define USE_HALIDE_BUFFERS_BUG_WORKAROUND true
    ...
    std::tuple<
        ...
        std::vector<std::tuple<int, int, int, int>>,
        std::vector<std::tuple<int, int, int, int>>>
    tiramisu::function::skewing_local_solver_positive(std::vector<tiramisu::computation *> fused_computations,
                                                      tiramisu::var outer_variable, tiramisu::var inner_variable, int nb_parallel)
    {
        DEBUG_FCT_NAME(3);
        DEBUG_INDENT(4);
    ...
    std::tuple<
        ...
        std::vector<std::tuple<int, int, int, int, int, int, int, int, int>>,
        std::vector<std::tuple<int, int, int, int, int, int, int, int, int>>>
    tiramisu::function::skewing_local_3D_solver_positive(
                ...
                if ((i_outer == 0) && (j_inner == 1))
                {
                    ...
                    if (std::get<0>(result_tmp).size() > 0)
                    {
                        ...
                        std::string transformation_in = "{[i0,i1,i2]->[i_0,i_1,i_2] : "
                                                        " i_" +
                                                        std::to_string(index_different) + " = i" + std::to_string(index_different) + " and "
                                                                                                                                     "i_" +
                                                        std::to_string(i_outer) + " = i" + std::to_string(i_outer) + "*" + std::to_string(std::get<0>(identity_values)) +
                                                        " + i" + std::to_string(j_inner) + "*" + std::to_string(std::get<1>(identity_values)) + " and "
                                                                                                                                                "i_" +
                                                        std::to_string(j_inner) + " = i" + std::to_string(i_outer) + "*" + std::to_string(std::get<2>(identity_values)) +
        ...
        isl_basic_map *transformation)
    {
        ...
        DEBUG(10, tiramisu::str_dump(" Extracted parameters are : " +
                                     std::to_string(v1[0]) + " " + std::to_string(v2[0]) + " " + std::to_string(v3[0]) + " | " + std::to_string(v1[1]) + " " + std::to_string(v2[1]) + " " + std::to_string(v3[1]) + " | " + std::to_string(v1[2]) + " " + std::to_string(v2[2]) + " " + std::to_string(v3[2])));
