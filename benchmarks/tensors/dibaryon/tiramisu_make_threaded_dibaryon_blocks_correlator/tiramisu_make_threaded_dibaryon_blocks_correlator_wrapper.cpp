...
#define RUN_CHECK 1
   ...
   int main(int, char **)
   {
      ...
      for (wnum = 0; wnum < Nw2Hex; wnum++)
      {
         for (q = 0; q < 2 * Nq; q++)
         {
            snk_color_weights_A1[index_2d(wnum, q, 2 * Nq)] = rand() % Nc;
            snk_color_weights_T1_r1[index_2d(wnum, q, 2 * Nq)] = rand() % Nc;
            snk_color_weights_T1_r2[index_2d(wnum, q, 2 * Nq)] = rand() % Nc;
            snk_color_weights_T1_r3[index_2d(wnum, q, 2 * Nq)] = rand() % Nc;
            snk_spin_weights_A1[index_2d(wnum, q, 2 * Nq)] = rand() % Ns;
            snk_spin_weights_T1_r1[index_2d(wnum, q, 2 * Nq)] = rand() % Ns;
            snk_spin_weights_T1_r2[index_2d(wnum, q, 2 * Nq)] = rand() % Ns;
            snk_spin_weights_T1_r3[index_2d(wnum, q, 2 * Nq)] = rand() % Ns;
         }
         snk_weights_A1[wnum] = 1.0;
         snk_weights_T1_r1[wnum] = 1.0;
         snk_weights_T1_r2[wnum] = 1.0;
