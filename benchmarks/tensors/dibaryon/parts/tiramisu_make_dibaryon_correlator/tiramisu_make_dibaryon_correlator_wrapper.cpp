...
int main(int, char **)
{
   ...
   double ref_C_r[Lt * Nsrc * Nsnk];
   double ref_C_i[Lt * Nsrc * Nsnk];
   ...
   for (int i = 0; i < Nt; i++)
      for (int j = 0; j < Nsnk; j++)
         ...
         {
            ref_C_r[index_3d(k, j, i, Nsnk, Nt)] = 0.0;
            ref_C_i[index_3d(k, j, i, Nsnk, Nt)] = 0.0;
            C_r(i, j, k) = 0.0;
            C_i(i, j, k) = 0.0;
         }
                     ...
                     for (int kSprime = 0; kSprime < Ns; kSprime++)
                        for (int x = 0; x < Vsnk; x++)
                           for (int t = 0; t < Nt; t++)
                           {
                              ...
                              B1_Blocal_r1_i(jSprime, jCprime, m, x, kSprime, kCprime, iSprime, iCprime, t) = v2;
                              B2_Blocal_r1_r(jSprime, jCprime, m, x, kSprime, kCprime, iSprime, iCprime, t) = v3;
                        ...
                        for (int x = 0; x < Vsnk; x++)
                           for (int x2 = 0; x2 < Vsnk; x2++)
                              for (int t = 0; t < Nt; t++)
                              {
                                 ...
                                 B1_Bsingle_r1_i(x2, jSprime, jCprime, m, x, kSprime, kCprime, iSprime, iCprime, t) = v2;
                                 B2_Bsingle_r1_r(x2, jSprime, jCprime, m, x, kSprime, kCprime, iSprime, iCprime, t) = v3;
                                 ...
                                 B1_Bdouble_r1_i(x2, iSprime, iCprime, m, x, kSprime, kCprime, jSprime, jCprime, t) = v6;
                                 B2_Bdouble_r1_r(x2, iSprime, iCprime, m, x, kSprime, kCprime, jSprime, jCprime, t) = v7;
   ...
   for (int i = 0; i < Vsnk; i++)
      for (int k = 0; k < Vsnk; k++)
         for (int j = 0; j < Nsnk; j++)
         {
            ...
            b_snk_psi_re(j, k, i) = v1;
            b_snk_psi_im(j, k, i) = v2;
         }
